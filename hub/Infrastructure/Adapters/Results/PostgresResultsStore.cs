...
public sealed class PostgresResultsStore(IConfiguration config) : IResultsStore
{
    ...
    private async Task EnsureCreatedAsync()
    {
        ...
        // Allow disabling auto-migrations if desired (default: on)
        var autoMigrate = Environment.GetEnvironmentVariable("HUB_RESULTS_PG_AUTO_MIGRATE");
        var doMigrate = string.IsNullOrEmpty(autoMigrate) || string.Equals(autoMigrate, "1", StringComparison.OrdinalIgnoreCase) || string.Equals(autoMigrate, "true", StringComparison.OrdinalIgnoreCase);
        if (doMigrate)
        {
            await PostgresSchemaMigrations.ApplyAsync(_connString, CancellationToken.None);
        }
        _initialized = true;
    }
