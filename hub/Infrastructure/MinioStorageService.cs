...
public sealed class MinioStorageService : IDisposable
{
    ...
    public MinioStorageService(IConfiguration config, ChunkedLogger<MinioStorageService> logger)
    {
        ...
        _logger.LogMilestone(EventCodes.Generic, "Client initialized: {Endpoint}, bucket: {Bucket}, SSL: {UseSSL}",
    ...
    public async Task EnsureBucketExistsAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            if (!exists)
            {
                ...
                _logger.LogMilestone(EventCodes.Generic, "Created bucket: {Bucket}", _bucketName);
            }
            else
            {
        ...
        {
            _logger.LogError(ex, EventCodes.Generic, "Failed to ensure bucket exists: {Bucket}", _bucketName);
            throw;
        }
    }
}
