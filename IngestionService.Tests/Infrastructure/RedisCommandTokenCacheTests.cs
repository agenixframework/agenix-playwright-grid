...
[TestFixture]
public class RedisCommandTokenCacheTests
{
    private Mock<IDatabase> _redisMock;
    private Mock<ILogger<RedisCommandTokenCache>> _loggerMock;
    ...
    public void SetUp()
    {
        ...
        _loggerMock = new Mock<ILogger<RedisCommandTokenCache>>();
    }
    ...
    {
        // Arrange
        var cache = new RedisCommandTokenCache(_redisMock.Object, null!, _ttl, true, 1000, _loggerMock.Object);
        ...
        // First call to populate in-memory
        await cache.GetOrCreateTokenAsync(message, kind);

        _redisMock.Invocations.Clear();

        // Act
        var hash = await cache.GetOrCreateTokenAsync(message, kind);
    ...
    {
        // Arrange
        var cache = new RedisCommandTokenCache(_redisMock.Object, null!, _ttl, false, 1000, _loggerMock.Object);
        var message = "Test command";
        var kind = "click";

        _redisMock.Setup(x => x.StringGetAsync(It.Is<RedisKey>(k => k.ToString().Contains("command_token")), CommandFlags.None))
            .ReturnsAsync((RedisValue)message);
