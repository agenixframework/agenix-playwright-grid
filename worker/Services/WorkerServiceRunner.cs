...
public sealed class WorkerServiceRunner
{
    public async Task RunAsync(string[] args)
    {
        ...
        {
            // Redact secrets
            string redact(string? s) => string.IsNullOrEmpty(s) ? "" : "***";
            var diag = new
            {
                ...
                PublicWs = new { Host = options.PublicWsHost ?? "(unset)", Port = options.PublicWsPort ?? "(unset)", Scheme = options.PublicWsScheme },
                Sidecar = new { Script = options.SidecarScript, ReadyTimeoutSeconds = options.SidecarReadyTimeoutSeconds },
                Labels = options.Labels.ToDictionary(k => k.Key, v => v.Value),
                PoolConfig = options.PoolConfig.ToDictionary(k => k.Key, v => v.Value)
            };
            var json = System.Text.Json.JsonSerializer.Serialize(diag, new System.Text.Json.JsonSerializerOptions { WriteIndented = true });
        ...
        try
        {
            ...
            var pkgJsonPath = System.IO.Path.Combine(baseDir, "node_modules", envPkg, "package.json");
            string installedVer = "(unknown)";
            if (System.IO.File.Exists(pkgJsonPath))
            {
                var jsonText = await System.IO.File.ReadAllTextAsync(pkgJsonPath);
                using var doc = JsonDocument.Parse(jsonText);
                if (doc.RootElement.TryGetProperty("version", out var v))
                    installedVer = v.GetString() ?? installedVer;
            }
            Console.WriteLine($"[worker] Playwright startup: package={envPkg}, envVersion={envVer}, installedVersion={installedVer}");
        }
        catch (Exception ex)
        {
