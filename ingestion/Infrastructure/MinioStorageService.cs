...
public sealed class MinioStorageService : IDisposable
{
    ...
    public MinioStorageService(IConfiguration config, ILogger<MinioStorageService> logger)
    {
        ...
        _logger.LogInformation("[MinIO] Client initialized: {Endpoint}, bucket: {Bucket}, SSL: {UseSSL}",
    ...
    {
        _client?.Dispose();
        _logger.LogDebug("[MinIO] Client disposed");
    }
        ...
        CancellationToken ct = default)
    {
        ...
        {
            _logger.LogError(ex, "[MinIO] Failed to upload artifact: {ObjectKey}", objectKey);
            throw;
        }
    }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug("[MinIO] Downloaded artifact: {ObjectKey} ({Size} bytes)",
        ...
        {
            _logger.LogError(ex, "[MinIO] Failed to download artifact: {ObjectKey}", objectKey);
            throw;
        }
    }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogDebug("[MinIO] Generated pre-signed URL for {ObjectKey}, expires in {Expiry}s",
        ...
        {
            _logger.LogError(ex, "[MinIO] Failed to generate pre-signed URL for {ObjectKey}", objectKey);
            throw;
        }
    }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            _logger.LogInformation("[MinIO] Deleted artifact: {ObjectKey}", objectKey);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[MinIO] Failed to delete artifact: {ObjectKey}", objectKey);
            throw;
        }
    }

    /// <summary>
    ///     Checks if bucket exists and creates it if not (idempotent).
    ///     Should be called on service startup to ensure bucket is available.
    /// </summary>
    /// <param name="ct">Cancellation token</param>
    public async Task EnsureBucketExistsAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            if (!exists)
            {
                ...
                _logger.LogInformation("[MinIO] Created bucket: {Bucket}", _bucketName);
            }
            else
            {
                _logger.LogDebug("[MinIO] Bucket already exists: {Bucket}", _bucketName);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[MinIO] Failed to ensure bucket exists: {Bucket}", _bucketName);
            throw;
        }
    }
}
